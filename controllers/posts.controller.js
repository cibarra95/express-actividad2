...
module.exports.delete = async (req, res, next) => {
  try {
    const postId = req.params.id; // Assuming the post ID is provided in the URL params
    const deletedPost = await Post.findByIdAndDelete(postId); // Find and delete the post by ID

    if (!deletedPost) {
      return res.status(404).json({ message: "Post not found" }); // Handle case where post does not exist
    }

    res.status(204).send(); // Successfully deleted, send no content response
  } catch (error) {
